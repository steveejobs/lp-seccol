export const heroContent = {
  title: 'Antecipe não conformidades. Proteja sua operação.',
  description:
    'Testes, manutenção e certificação em áreas limpas e equipamentos críticos para identificar desvios e produzir evidências técnicas antes da fiscalização.',
  primaryAction: {
    label: 'Iniciar diagnóstico técnico',
    href: '/contato',
  },
  secondaryAction: {
    label: 'Ver evidências técnicas',
    href: '#evidencias',
  },
  evidence: [
    { value: 'Desde 2009', label: 'Experiência aplicada ao controle ambiental' },
    { value: 'Atuação nacional', label: 'Serviços técnicos em todo o país' },
    {
      value: 'Registros oficiais',
      label: 'CREA, SBCC e Vigilância Sanitária',
    },
  ],
} as const
